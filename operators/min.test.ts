...
describe("min tests", () => {
  ...
  describe("string", () => {
    ...
    it("when value is string and length less than model should return error-string", () => {
      expect(min(4)("Bye")).toBe(
        "Length of string should be greater or equal 4, current length - 3",
      );
    });
  });
  describe("Date", () => {
    ...
    it("when value is Date and less than model should return error-string", () => {
      expect(min(new Date(dtEqual))(new Date(dtLess))).toBe(
        "Value should be greater or equal " + dtEqual,
      );
    });
    it("when value is not Date, but model is Date, should return error-string", () => {
      expect(min(new Date(dtEqual))("" as any)).toBe(
  ...
  describe("array", () => {
    ...
    it("when value is array and length less than model should return error-string", () => {
      expect(min(4)([1, 2])).toBe(
        "Length of array should be greater or equal 4, current length - 2",
      );
    });
  });
});
