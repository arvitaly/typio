  ...
  const fn = ((value: string | number | Date | any[]) => {
    if (maxValue instanceof Date) {
      ...
      return value.getTime() <= maxValue.getTime()
        ? true
        : "Value should be less or equal " + maxValue.toUTCString();
    }
    if (typeof value === "string") {
      return value.length <= maxValue
        ? true
        : "Length of string should be less or equal " +
            maxValue +
            ", current length - " +
            value.length;
    }
    if (Array.isArray(value)) {
      return value.length <= maxValue
        ? true
        : "Length of array should be less or equal " +
            maxValue +
            ", current length - " +
            value.length;
    }
    return value <= maxValue
      ? true
      : "Value should be greater or equal " + maxValue;
  }) as any;
  fn.label = "max";
  return fn;
};
