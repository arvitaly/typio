...
describe("max tests", () => {
  ...
  describe("string", () => {
    ...
    it("when value is string and length greater than model should return error-string", () => {
      expect(max(4)("Hello")).toBe(
        "Length of string should be less or equal 4, current length - 5",
      );
    });
  });
  describe("Date", () => {
    ...
    it("when value is Date and less or equal model should return true", () => {
      expect(max(new Date(dtEqual))(new Date(dtEqual))).toBe(true);
      expect(max(new Date(dtEqual))(new Date(dtLess))).toBe(true);
    });
    it("when value is Date and greater than model should return error-string", () => {
      expect(max(new Date(dtEqual))(new Date(dtGreater))).toBe(
        "Value should be less or equal " + dtEqual,
      );
    });
    it("when value is not Date, but model is Date, should return error-string", () => {
      expect(max(new Date(dtEqual))("" as any)).toBe(
        "Value should be instance of Date",
      );
    });
  });
  describe("array", () => {
    ...
    it("when value is array and length greater than model should return error-string", () => {
      expect(max(4)([1, 2, 3, 4, 5])).toBe(
        "Length of array should be less or equal 4, current length - 5",
      );
    });
  });
});
